...
bool NetworkCodingEncoderService::writeCodedBlocks(FILE* _pnetworkCodedBlockFile,
    ...
    for (int cnt = 0; cnt < numberOfBlocks; cnt++) {
        ...
        printf("Encoding block with coefficients size: %d\n", coeffsSize);
    ...
    printf("Encoding completed - wrote %d bytes\n", totalBytesWritten);
    return true;
}
...
DataObjectRef NetworkCodingEncoderService::encodeDataObject(
    ...
    if (!addedAttributes) {
        printf("Unable to add attributes\n");
        return NULL;
    }
    ...
    //printf("Adding send event for data object %s\n", originalId);
    printf("Added network coded data object file %s\n",
            networkCodedDataObjectRef->getFilePath().c_str());
    printf("Adding parent data object %s to data object storage\n", originalStringId.c_str());
...
DataObjectRef NetworkCodingEncoderService::encodeFragment(
	...
	printf("parent object size is %d\n", parentDataObjectFileSize);
	...
	printf("frag start position is %d\n", fragmentStartPosition);
	printf("frag actual size is %d\n", actualFragmentSize);
...
bool NetworkCodingEncoderService::addAttributes(DataObjectRef originalDataObject,
    ...
    for (Attributes::const_iterator it = originalAttributes->begin();
        ...
        if (!addAttribute) {
            printf("Unable to add attribute\n");
            return false;
        }
    }
    ...
    if (!addedIsNetworkedCodedAttribute) {
        printf("Unable to add networkcoding attribute\n");
        return false;
    }
    ...
    if(len == 0) {
        printf("Orignal data len is zero - file already deleted\n");
        return false;
    }
    ...
    if (!addedDataLenAttribute) {
        printf("Unable to add orignal data len attribute\n");
        return false;
    }
    ...
    bool addedIdAttribute = networkCodedDataObject->addAttribute(HAGGLE_ATTR_PARENT_DATAOBJECT_ID,
            originalStringId, 0);
    if (!addedIdAttribute) {
        printf("Unable to add original data object id attribute\n");
        return false;
    }

    //add dataobject name
    string originalName = networkCodingDataObjectUtility->getFileName(originalDataObject);
    printf("Add original file name %s as attribute\n", originalName.c_str());
    ...
    if (!addedNameAttribute) {
        printf("Unable to add original file name attribute\n");
        return false;
    }

    //add create time
    string originalCreateTime = originalDataObject->getCreateTime().getAsString();
    printf("Add original createtTime %s as attribute\n", originalCreateTime.c_str());
    ...
    if (!addedCreatedTimeAttribute) {
        printf("Unable to add original create time attribute\n");
        return false;
    }
    ...
    if(originalDataObject->getSignature()) { // MOS
      ...
      printf("Add original signee %s as attribute\n", parentSignee.c_str());
      ...
      if(!addedSigneeAttribute) {
        printf("Unable to add original signee attribute\n");
        return false;
      }
      ...
      if (base64_encode_alloc(
        ...
        printf("Unable to generate base64 encoded signature\n");
        return false;
      }
      string parentSignature = base64_signature;
      printf("Add original signature %s as attribute\n", parentSignature.c_str());
      ...
      if(!addedSignatureAttribute) {
        printf("Unable to add original signature as attribute\n");
        return false;
      }
      if(base64_signature) {
...
bool NetworkCodingEncoderService::addAttributesFromFragDataObject(DataObjectRef fragDataObject,
    ...
    printf("Add original createTime %s as attribute\n", originalCreateTime.c_str());
    bool addedCreatedTimeAttribute = networkCodedDataObject->addAttribute(
            HAGGLE_ATTR_NETWORKCODING_PARENT_CREATION_TIME, originalCreateTime, 0);
    if (!addedCreatedTimeAttribute) {
        printf("Unable to add original create time attribute\n");
